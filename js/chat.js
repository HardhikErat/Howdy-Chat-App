...
database.child(`User/${currentuser}/${recieverId}/Messages`).on(`child_added`, messages => {
    ...
    if (message.senderIs === `true`) {
      mess.style = `text-align: right;color: black; word-wrap: break-word; margin-left: 150px; border-radius: 5px; font-family: impect;background-color: #BDBABA ` 
    }
    if (message.senderIs === `false`) {
      mess.style = `color: black; background-color: #DCDAF2; word-wrap: break-word; border-radius: 5px;  width: 200px; font-family: impect;`
    }
  })